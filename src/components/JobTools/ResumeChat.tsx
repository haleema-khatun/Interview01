...
export const ResumeChat: React.FC = () => (
  <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-10 border border-gray-200 dark:border-gray-700 text-center">
      ...
      <p className="text-gray-600 dark:text-gray-300 text-lg">Please wait for some time.</p>
    </div>
  </div>
);
...
// import { ArrowLeft, Upload, FileText, Send, User, Bot, Loader, Brain, AlertCircle, Zap, MessageSquare, Briefcase, GraduationCap, Code, Award, Download, Copy, Trash2, CheckCircle } from 'lucide-react';
// import { useNavigate } from 'react-router-dom';
// import { TextExtractionService } from '../../services/textExtractionService';
// import { AIEvaluationService } from '../../services/ai/aiEvaluationService';
// import { useTheme } from '../../contexts/ThemeContext';
// import toast from 'react-hot-toast';
// import { RAGResumeService } from '../../services/ragResumeService';
...
//     }
//     // else, do nothing (user has scrolled up)
//   }, [messages]);
...
//         // --- RAG: Chunk, embed, and store resume chunks ---
//         await RAGResumeService.embedAndStoreChunks(cleanedText);
...
//       let ragContext = '';
//       try {
//         const relevantChunks = await RAGResumeService.retrieveRelevantChunks(currentInput, 4);
//         if (relevantChunks.length > 0) {
//           ragContext = relevantChunks.map((c, i) => `Resume Section ${i + 1}:\n${c.text}`).join('\n\n');
//         }
//       } catch (err) {
...
//       if (hasAnyApiKey) {
//         // --- RAG prompt ---
//         const prompt = `You are an AI assistant. You must answer ONLY using the information in the provided resume sections below.
// If the answer is not present, say: "I could not find that information in your resume."
...
// Remember: Do not make up information. Only use the resume context above.`;
//         try {
//           const response = await AIEvaluationService.evaluateWithBestAvailable({
//             question: 'Resume chat conversation (RAG)',
//             answer: prompt,
//             ratingMode: 'lenient'
//           });
//           let aiResponse = '';
...
//           }
//           aiResponse = aiResponse.replace(/^```/, '').replace(/```$/, '').replace(/^ASSISTANT:/, '').trim();
//           setMessages(prev => [...prev, {
//             id: (Date.now() + 1).toString(),
//             type: 'assistant',
//             content: aiResponse,
...
//           }]);
//         } catch (aiError) {
//           console.warn('AI conversation failed:', aiError);
